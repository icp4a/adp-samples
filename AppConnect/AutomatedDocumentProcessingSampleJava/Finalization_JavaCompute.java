...
public class Finalization_JavaCompute extends MbJavaComputeNode {
	...
	public void evaluate(MbMessageAssembly inAssembly) throws MbException
	{
		...
		try {
			 ...
			 * Verify HMAC
			 */
			MbElement dataElement = inMessage.getRootElement().getFirstElementByPath("JSON/Data");
			boolean verified = HMACAuthentication.verifyHMACHeader(hmacValue, dataElement.toBitstream(null, null, null, 0, 0, 0));
    ...
    private void validateEnvironment()
    {
        ...
        if (System.getenv(Constants.ZEN_ENABLED) == null) {
        	throw new RuntimeException("Environment variable " + Constants.ZEN_ENABLED + " is not set");
        }
        if (System.getenv(Constants.ZEN_FRONT_DOOR_URL) == null) {
        	throw new RuntimeException("Environment variable " + Constants.ZEN_FRONT_DOOR_URL + " is not set");
        }
        if (System.getenv(Constants.IDP_SSO_URL) == null) {
        	throw new RuntimeException("Environment variable " + Constants.IDP_SSO_URL + " is not set");
        }
