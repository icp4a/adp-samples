...
def downloadFile(configuration_settings, token, analyzerId, output, output_path, filename):
    ...
    if output.lower() == "json" and configuration_settings.get("download_json_option") and configuration_settings["download_json_option"] == "basic":
        get_url = get_url + '?outputJSON=basic'
    logger.debug('Download {} url: {}'.format(output.lower(), get_url))
    try:
        response = requests.request("GET", get_url, headers=headers, verify=configuration_settings['ssl_verification'])
        # print(response.text)
        if response.status_code >= 400:
